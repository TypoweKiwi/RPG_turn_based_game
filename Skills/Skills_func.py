...
def heal_func(stats_dict, target):
    ...
    skill_power = 3
    heal_amount = skill_power*int(ability_power*0.7)
...
def heavy_strike_func(stats_dict, target):
    skill_damage = 2
    apply_damage(skill_damage=skill_damage, modifier=stats_dict["AD"], target=target, damage_type=DamageType.physical, criticality=True, crit_chance=(stats_dict["Crit"]/2))
